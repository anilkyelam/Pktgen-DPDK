...
/**************************************************************************//**
 *
 * pktgen_pktcfg_apply - Randomly select a pktconfig (based on weights) and 
 * apply to the each packet.
 *
 * This method should be in its own file but I'm lazy! Don't be me.
 * 
 * DESCRIPTION
 * Randomly select a pktconfig (based on weights) and 
 * apply to the each packet.
 *
 * RETURNS: N/A
 *
...
void
pktgen_pktcfg_apply(port_info_t *info, struct rte_mbuf **pkts, size_t cnt, int32_t seq_idx)
{
    size_t mbuf_cnt;
    struct pg_ether_hdr *ehdr;
    uint64_t rnd_raw, rnd, acm = 0;
    int i;
    int lid = rte_lcore_id();
    int qid = get_txque(pktgen.l2p, lid, info->pid);
    char* p;
    uint32_t port_range;
    struct pg_udp_hdr* uhdr;
    struct pg_tcp_hdr* thdr;

    for (mbuf_cnt = 0; mbuf_cnt < cnt; ++mbuf_cnt) {
        ehdr = rte_pktmbuf_mtod(pkts[mbuf_cnt], struct pg_ether_hdr *);

        /* Randomly pick a pktconfig proportinal to their weights */		
        rnd_raw = rand();
        rnd = rnd_raw % info->pkt_cfgs.sum_wt;
        acm = 0;
        for (i = 0; i < info->pkt_cfgs.count; i++ ) {
            acm += info->pkt_cfgs.weights[i];
            if (rnd < acm)
                break;
        }
        // pktgen_log_warning("Applying pkt config %d, rnd: %d, entries: %d, sum: %d", 
        // 	i, rnd, info->pkt_cfgs.count, info->pkt_cfgs.sum_wt);

        /* Apply the pkt config. Just the dst mac addr for now */
        pg_ether_addr_copy(&info->pkt_cfgs.eth_dst_addrs[i], &ehdr->d_addr);
        info->pkt_cfgs.pc[qid].txpkts[i]++;
        info->pkt_cfgs.pc[qid].txbytes[i] += pkts[mbuf_cnt]->pkt_len;

        /* Randomize UDP or TCP dst port */
        /* Actually, change ports deterministically. Calling rand() from multiple cores seem to have sync issues */
        if (1) 
        {
            p = rte_pktmbuf_mtod(pkts[mbuf_cnt], char *);
            p += sizeof(struct pg_ether_hdr);
            p += (info->seq_pkt[seq_idx].ethType == PG_ETHER_TYPE_IPv4) ? sizeof(struct pg_ipv4_hdr) : sizeof(struct pg_ipv6_hdr);
            port_range = 2; //(i == 0) ? 10 : 1000;
            if (info->pkt_cfgs.pc[qid].port_incr == 0)	info->pkt_cfgs.pc[qid].port_incr = (qid+1);
            if (info->seq_pkt[seq_idx].ipProto == PG_IPPROTO_UDP) {
                uhdr = (struct pg_udp_hdr*) p;
                //pktgen_log_warning("pktgen udp dport before: %d", ntohs(uhdr->dst_port));
                if (info->pkt_cfgs.pc[qid].port_incr >= port_range)	info->pkt_cfgs.pc[qid].port_incr %= port_range;
                uhdr->dst_port = htons(5000 + info->pkt_cfgs.pc[qid].port_incr);
                info->pkt_cfgs.pc[qid].port_incr += 1;
                //info->pkt_cfgs.pc[qid].port_incr += (qid+1);
            }
            else {
                thdr = (struct pg_tcp_hdr*) p;
                if (info->pkt_cfgs.pc[qid].port_incr >= port_range)	info->pkt_cfgs.pc[qid].port_incr %= port_range;
                thdr->dst_port = htons(5000 + info->pkt_cfgs.pc[qid].port_incr);
                info->pkt_cfgs.pc[qid].port_incr += (qid+1);
            }
        }
    }
...
pktgen_send_burst(port_info_t *info, uint16_t qid)
{
    ...
    uint32_t ret, cnt, tap, rnd, tstamp, i, pktcfg;
    int32_t seq_idx;
    ...
    pktcfg = pktgen_tst_port_flags(info, APPLYING_PKT_CONFIGS);
    ...
    while (cnt && pktgen_tst_port_flags(info, SENDING_PACKETS)) {
        ...
        if (pktcfg)
            pktgen_pktcfg_apply(info, pkts, cnt, seq_idx);
...
pktgen_recv_tstamp(port_info_t *info, struct rte_mbuf **pkts, uint16_t nb_pkts)
{
    ...
    for (i = 0; i < nb_pkts; i++) {
        ...
        if (pktcfg) {
            ehdr = rte_pktmbuf_mtod(pkts[i], struct pg_ether_hdr *);
            cfgid = 0;

            /* Determine pktcfg based on src mac (it'll be swapped) */
            for (j = 0; j < info->pkt_cfgs.count; j++ ) {
                if (rte_is_same_ether_addr(&info->pkt_cfgs.eth_dst_addrs[j], &ehdr->s_addr)) {
                    info->pkt_cfgs.pc[qid].rxpkts[j]++;
                    info->pkt_cfgs.pc[qid].rxbytes[j] += pkts[i]->pkt_len;
                    // pktgen_log_warning("Got pkt of config %d, total rx pkts on queue %d: (%lu, %lu)\n",
                    // 	j, qid, info->pkt_cfgs.rxpkts[qid][j], info->pkt_cfgs.rxbytes[qid][j]);
                    cfgid = j+1;
                }
            }
        }
...
/**************************************************************************//**
 *
 * pktgen_recv_pktcfg - Determine applied pktconfig on a packet and increment stats.
 *
 * This method should be in its own file but I'm lazy! Don't be me.
 * 
 * DESCRIPTION
...
void
pktgen_recv_pktcfg(port_info_t *info, struct rte_mbuf **pkts, size_t cnt)
{
    size_t mbuf_cnt;
    struct pg_ether_hdr *ehdr;
    int i;
    ...
    for (mbuf_cnt = 0; mbuf_cnt < cnt; ++mbuf_cnt) {
        ...
        /* Determine pktcfg based on src mac (it'll be swapped) */
        for (i = 0; i < info->pkt_cfgs.count; i++ ) {
            if (rte_is_same_ether_addr(&info->pkt_cfgs.eth_dst_addrs[i], &ehdr->s_addr)) {
                info->pkt_cfgs.pc[qid].rxpkts[i]++;
                info->pkt_cfgs.pc[qid].rxbytes[i] += pkts[mbuf_cnt]->pkt_len;
                // pktgen_log_warning("Got pkt of config %d, total rx pkts on queue %d: (%lu, %lu)\n",
                // 	i, qid, info->pkt_cfgs.rxpkts[qid][i], info->pkt_cfgs.rxbytes[qid][i]);
                return;
            }
        }
    }
