...
#define MAX_PKTCFG_ENTRIES 	10
#define MAX_PKTCFG_CORES 	10
...
enum {						/* Per port flag bits */
    ...
    APPLYING_PKT_CONFIGS	= (1 << 27),	/**< Applying pkt configs to tx packets > */
...
typedef struct {
    uint64_t txpkts[MAX_PKTCFG_ENTRIES];
    uint64_t txbytes[MAX_PKTCFG_ENTRIES];
    uint64_t rxbytes[MAX_PKTCFG_ENTRIES];
    uint64_t rxpkts[MAX_PKTCFG_ENTRIES];
    uint32_t port_incr;						/** Next port increment when randomizing L4 ports  */
} pktcfg_percore_t __rte_cache_aligned;

typedef struct {
    uint8_t count;													/** Number of configs */	
    struct pg_ether_addr eth_dst_addrs[MAX_PKTCFG_ENTRIES];			/** Eth mac addrs for this config */	
    uint16_t weights[MAX_PKTCFG_ENTRIES];							/** Weight for each config */
    uint16_t sum_wt;
    pktcfg_percore_t pc[MAX_PKTCFG_CORES];							/** Per-Core Counters */
} pkt_config_t;
