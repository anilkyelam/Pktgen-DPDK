 ...
 *
 * 
 * pktgen_pktcfg_add - Add a pktcfg entry.
 *
 * DESCRIPTION
 * Add a pktcfg entry.
 *
...
static int
pktgen_pktcfg_add(lua_State *L)
{
    portlist_t portlist;
    struct pg_ether_addr mac;

    switch (lua_gettop(L) ) {
    default: return luaL_error(L, "pktcfg_add, wrong number of arguments");
    case 3:
        break;
    }
    ...
    foreach_port(portlist, single_add_pktcfg(info, &mac, (uint32_t)luaL_checkinteger(L, 3)));
 ...
 *
 * 
 * pktgen_pktcfg_rst - Clear all pktcfg entries.
 *
 * DESCRIPTION
 * Clear all pktcfg entries.
 *
 * RETURNS: N/A
 *
 * SEE ALSO:
...
static int
pktgen_pktcfg_rst(lua_State *L)
{
    portlist_t portlist;

    switch (lua_gettop(L) ) {
    default: return luaL_error(L, "pktcfg_rst, wrong number of arguments");
    case 1:
        break;
    }

    portlist_parse(luaL_checkstring(L, 1), &portlist);
    foreach_port(portlist, single_reset_pktcfg(info));
...
/**************************************************************************//**
 *
 * pktgen_pktcfg - Enable or Disable pktcfg.
 *
 * DESCRIPTION
 * Enable or disable pktcfg.
 *
 * RETURNS: N/A
 *
 * SEE ALSO:
...
static int
pktgen_pktcfg(lua_State *L)
{
    portlist_t portlist;

    switch (lua_gettop(L) ) {
    default: return luaL_error(L, "pktcfg, wrong number of arguments");
    case 2:
        break;
    }
    portlist_parse(luaL_checkstring(L, 1), &portlist);
    ...
    foreach_port(portlist,
             enable_pktcfg(info, estate((char *)luaL_checkstring(L, 2))));
...
{
    eth_stats_t stats;
    eth_stats_t pktcfg_stats[MAX_PKTCFG_ENTRIES];
    char name[30];
    int i;

    memset(pktcfg_stats, 0, sizeof(pktcfg_stats));

    if (strcmp(type, "pktcfg") == 0)	pktgen_port_stats(info->pid, "port", &stats);
    else	pktgen_port_stats(info->pid, type, &stats);
    ...
    if (strcmp(type, "pktcfg") == 0) {
        pktgen_port_pktcfg_stats(info->pid, pktcfg_stats);
        // for (i = 0; i < info->pkt_cfgs.count; i++) {
        // Print stats for 2 entries for now, regardless of whether both are being used.
        for (i = 0; i < 10; i++) {			
            sprintf(name, "cfg%d_ipackets", i+1);
            printf("%s: %lu\n", name, pktcfg_stats[i].ipackets);
            setf_integer(L, name, pktcfg_stats[i].ipackets);
            printf("%s: %lu\n", name, pktcfg_stats[i].opackets);
            sprintf(name, "cfg%d_opackets", i+1);
            setf_integer(L, name, pktcfg_stats[i].opackets);
            printf("%s: %lu\n", name, pktcfg_stats[i].ibytes);
            sprintf(name, "cfg%d_ibytes", i+1);
            setf_integer(L, name, pktcfg_stats[i].ibytes);
            printf("%s: %lu\n", name, pktcfg_stats[i].obytes);
            sprintf(name, "cfg%d_obytes", i+1);
            setf_integer(L, name, pktcfg_stats[i].obytes);
        }
    }
...
static const luaL_Reg pktgenlib[] = {
    ...
    {"pktcfg_add",	pktgen_pktcfg_add},		/* add a pktcfg entry */
    {"pktcfg_rst",	pktgen_pktcfg_rst},		/* clear all pktcfg entries */
    {"pktcfg",		pktgen_pktcfg},			/* enable or disable pktcfg */
