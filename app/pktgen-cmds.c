...
/**************************************************************************//**
 *
 * pktgen_port_pktcfg_stats - Get the aggregated pktcfg stats for a given port
 *
 * DESCRIPTION
 *Get the aggregated pktcfg stats for a given port.
 *
 * RETURNS: N/A
 *
 * SEE ALSO:
...
int
pktgen_port_pktcfg_stats(int port, eth_stats_t *pstats)
{
    port_info_t *info = &pktgen.info[port];
    int txq, rxq, i, j;
    uint64_t txp, txb, rxp, rxb;

    txq = get_port_txcnt(pktgen.l2p, info->pid);
    rxq = get_port_rxcnt(pktgen.l2p, info->pid);
    for (i = 0; i < info->pkt_cfgs.count; i++ )
    {
        txp = txb = rxp = rxb = 0;
        for (j = 0; j < txq; j++){	txp += info->pkt_cfgs.pc[j].txpkts[i];	txb += info->pkt_cfgs.pc[j].txbytes[i];	}
        for (j = 0; j < rxq; j++){	rxp += info->pkt_cfgs.pc[j].rxpkts[i];	rxb += info->pkt_cfgs.pc[j].rxbytes[i];	}
        pstats[i].ibytes = rxb;
        pstats[i].ipackets = rxp;
        pstats[i].obytes = txb;
        pstats[i].opackets = txp;
    }
...
pktgen_stop_latency_sampler(port_info_t *info)
{
    ...
    if (outfile == NULL)
    {
        ...
        /* If pktconfigs are being applied, try classifying packets fr different cfgs 
         * write them to different files */
        if (pktgen_tst_port_flags(info, APPLYING_PKT_CONFIGS))
        {
            for (cfg = 1; cfg <= MAX_PKTCFG_ENTRIES; cfg++) 
            {
                int first_entry = 1;
                sprintf(fname, "%s_%d", info->latsamp_outfile, cfg);
                for (q = 0, count = 0; q < rxq; q++) {
                    for (i = 0; i < info->latsamp_stats[q].idx; i++) {
                        if (info->latsamp_stats[q].cfgid[i] == cfg) {
                            if (first_entry) {
                                /* Create file */
                ...
                if (count > 0) {
                    pktgen_log_warning("Wrote %d sample latencies for pktcfg %d to file %s", count, cfg, fname);
                    fclose(outfile);
                }
            }
        }
...
/**************************************************************************//**
 *
 * enable_` - Enable or disable applying pktconfig entries to sending packets.
 *
 * RETURNS: N/A
 *
 * SEE ALSO:
...
void
enable_pktcfg(port_info_t *info, uint32_t state)
{
    int txq, rxq;
    ...
    if (state == ENABLE_STATE) {	
        if (!pktgen_tst_port_flags(info, APPLYING_PKT_CONFIGS)) {
            ...
            if (info->pkt_cfgs.count == 0) {
                pktgen_log_warning("No pktconfigs to apply. Add at least one entry!");
                return;
            }

            txq = get_port_txcnt(pktgen.l2p, info->pid);
            rxq = get_port_rxcnt(pktgen.l2p, info->pid);
            if (txq > MAX_PKTCFG_CORES || rxq > MAX_PKTCFG_CORES) {
                pktgen_log_error("ERROR! The port is configured with more tx (%d) or rx (%d) cores than pktcfg can handle (%d)!", 
                    txq, rxq, MAX_PKTCFG_CORES);
                return;
            }

            pktgen_set_port_flags(info, APPLYING_PKT_CONFIGS);
        }
    } else {
        if (pktgen_tst_port_flags(info, APPLYING_PKT_CONFIGS)) {
            pktgen_clr_port_flags(info, APPLYING_PKT_CONFIGS);

            /* Clear counters */
            memset(info->pkt_cfgs.pc, 0, sizeof(info->pkt_cfgs.pc));
        }
    }
}
...
/**************************************************************************//**
 *
 * single_add_pktcfg - Add a packet config entry
 *
 * RETURNS: N/A
 *
 * SEE ALSO:
...
void
single_add_pktcfg(port_info_t *info, struct pg_ether_addr *mac, uint32_t weight)
{
    /* Stop if pktcfg is enabled */
    if (pktgen_tst_port_flags(info, APPLYING_PKT_CONFIGS)) {
        pktgen_log_warning("Pktcfg is enabled, disable it first to edit!");
        return;
    }

    /* Other checks */
    if (info->pkt_cfgs.count >= MAX_PKTCFG_ENTRIES - 1) {
        pktgen_log_warning("Cannot add pktconfig, reached max entries!");
        return;
    }
    if (weight <= 0 || weight > 100) {
        pktgen_log_warning("Weight should be between 1 and 100!");
        return;
    }

    memcpy(&info->pkt_cfgs.eth_dst_addrs[info->pkt_cfgs.count], mac, 6);
    info->pkt_cfgs.weights[info->pkt_cfgs.count] = weight;
    info->pkt_cfgs.count++;
    info->pkt_cfgs.sum_wt += weight;
    pktgen_set_tx_update(info);
    pktgen_log_warning("Pktcfg now has %d entries. Total weight: %d", 
        info->pkt_cfgs.count, info->pkt_cfgs.sum_wt);
}


/**************************************************************************//**
 *
 * single_reset_pktcfg - Remove all pkt config entries
 *
 * RETURNS: N/A
 *
 * SEE ALSO:
...
void
single_reset_pktcfg(port_info_t *info)
{
    /* Stop if pktcfg is enabled */
    if (pktgen_tst_port_flags(info, APPLYING_PKT_CONFIGS)) {
        pktgen_log_warning("Pktcfg is enabled, disable it first to reset!");
        return;
    }

    info->pkt_cfgs.count = 0;
    info->pkt_cfgs.sum_wt = 0;
    pktgen_set_tx_update(info);
}
